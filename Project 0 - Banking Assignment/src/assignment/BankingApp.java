...
public class BankingApp {
	public static void main(String[] args) {
		...
		// Reading off the users.txt
		service.users = DAO.readAllUsers();
		printAll(service.users);
		
		// Generate a users.txt file, fill it with pre-made Users, then sync the list and file together
//		ArrayList<User> list = service.getAllLocal();
//		DAO.syncUsers(list);
	...
	static void signUp() {
		...
		User u = new User(username, firstname, lastname, password, 0);
		Service.addUser(u);
	...
	static void updateBalance(User u, int option) {
		...
		if(option == 0) { // option for depositing money
			Service.deposit(u.getUsername(), amt);
			System.out.println("Money was deposited");
		}
		else if(option == 1) { // option for withdrawing money
			boolean b = Service.withdraw(u.getUsername(), amt);
			if(b) System.out.println("Successfully withdrew money");
			else System.out.println("You can't withdraw that amount");
		}
	}
